 ...
 * @package TorneLIB
 */
class libdriver_mysql implements libdriver_interface {
	...
	function __destruct() {
		...
		} else if ( $this->getDriverType() === TORNEVALL_DATABASE_DRIVERS::DRIVER_MYSQL_DEPRECATED ) {
			try {
	...
	public function setConfig( $configJsonFile = '/etc/tornevall_config.json' ) {
		...
		if ( $configJsonFile != "/etc/tornevall_config.json" && ! file_exists( $configJsonFile ) ) {
			throw new \Exception( "File $configJsonFile is missing", TORNEVALL_DATABASE_EXCEPTIONS::DRIVER_CONFIGURATION_MISSING );
		}
	}
	...
	private function CONNECT_PDO() {
		...
		if ( class_exists( 'PDO' ) ) {
			$pdoDriversStatic = \PDO::getAvailableDrivers();
			if ( ! in_array( 'mysql', $pdoDriversStatic ) ) {
				throw new \Exception( "You are trying to use a database driver that does not exist (PDO)", TORNEVALL_DATABASE_EXCEPTIONS::DRIVER_TYPE_MYSQLP_NOT_EXIST );
			}
			$DSN       = 'mysql:dbname=' . $this->getDatabase() . ';host=' . $this->getServerHostAddr();
			$this->PDO = new \PDO( $DSN, $this->getServerUserName(), $this->getServerPassword(), $this->getServerOptions() );
			if ( is_object( $this->PDO ) ) {
				...
				throw new \Exception( "Could not connect to PDO", TORNEVALL_DATABASE_EXCEPTIONS::DRIVER_TYPE_MYSQLP_CONNECT_UNKNOWN_ERROR );
			}
		}
	...
	private function CONNECT_MYSQL_DEPRECATED() {
		...
		if ( function_exists( 'mysql_connect' ) ) {
			...
			$connectResource = @mysql_connect( $this->getServerHostAddr(), $this->getServerUserName(), $this->getServerPassword(), true );
			if ( mysql_errno() ) {
			...
			throw new \Exception( "You are trying to use a database driver that does not exist (mysql_connect)", TORNEVALL_DATABASE_EXCEPTIONS::DRIVER_TYPE_MYSQLD_NOT_EXIST );
		}
	...
	public function connect( $serverIdentifier = '', $serverOptions = array(), $serverHostAddr = null, $serverUsername = null, $serverPassword = null ) {
		...
		} else if ( $driverInit == TORNEVALL_DATABASE_DRIVERS::DRIVER_MYSQL_PDO ) {
	...
	private function QUERY_MYSQLI_PREPARE( $queryString = '', $parameters = array(), $tests = array() ) {
		...
		if ( ! empty( $statementPrepare ) ) {
			$refArgs = array( $statementPrepare, $this->getParameters( $parameters ) );
			...
			if ( version_compare( phpversion(), "5.3.0", ">=" ) ) {
				...
				if ( mysqli_stmt_execute( $statementPrepare ) ) {
					...
					if ( method_exists( $statementPrepare, "get_result" ) && ( ! isset( $tests['META'] ) && ! in_array( 'META', $tests ) ) ) {
	...
	public function query_raw( $queryString = '' ) {
		if ( ! empty( $queryString ) ) {
			...
			throw new \Exception( "Can not parse empty query string", TORNEVALL_DATABASE_EXCEPTIONS::DRIVER_EMPTY_QUERY );
		}
	}
	...
	public function fetch( $resource = null, $columnArray = true ) {
		...
		} else if ( is_object( $this->mysqlPreparedResult ) ) {
			if ( $this->getDriverType() === TORNEVALL_DATABASE_DRIVERS::DRIVER_MYSQL_IMPROVED ) {
				if ( ! $this->getAsSqlObject ) {
	...
	public function escape( $inputString = '' ) {
		...
		} else if ( $this->getDriverType() === TORNEVALL_DATABASE_DRIVERS::DRIVER_MYSQL_IMPROVED ) {
			$returnString = @mysql_real_escape_string( $this->dataResource, $this->escape_deprecated( $injectionString ) );
		}
